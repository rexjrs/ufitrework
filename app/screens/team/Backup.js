...
export default class Team extends Component {
    ...
    addComment(id){
        let comment = this.state['comment'+id]
        if(comment == ""){
            ...
            for(var i in tempArray){
                if(tempArray[i].id == id){
                    let commentArray = tempArray[i].comments
                    commentArray.push({
                        username: this.state.username,
                        comment: comment
                    })
    ...
    _renderRow(value){
        ...
        var Comments =  value.comments.map((b,i) => {
            return (
                <Text key={i} style={{color: "#666666", fontSize: 14, paddingLeft: 20}}><Text style={{fontWeight: "bold"}}>{b.username}</Text> {b.comment}</Text>
            )
        });
        return(
                    ...
                    <View style={[styles.commentHeader,{height: 30,flex:0.8, zIndex: 5}]}>
                        ...
                        <View style={{flex:0.9}}>
                                <TextInput
                                    ...
                                    multiline={true}
                                    blurOnSubmit={true}
                                    ref={component => this['comment'+value.id] = component}
                                    onChangeText={(commentInput) => {this.setState({["comment"+value.id]: commentInput})}}
                                />
                        </View>
                        <View style={{justifyContent: "center", marginRight: 20}}>
                            <TouchableOpacity style={{paddingLeft: 10}} onPress={() => {this['comment'+value.id].setNativeProps({text: ''});this.addComment(value.id)}}>
